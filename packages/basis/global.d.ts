// glyphdraw 解耦：共享 ambient 声明，替代上游 `@excalidraw/excalidraw/global`。
// basis 包（common/element/math/utils）的 global.d.ts 通过
// `import "@excalidraw/excalidraw/global"`（tsconfig 重映射到本文件）加载这些声明。

// Minimal "process" stub so code guarded by `typeof process !== "undefined"` compiles
// without pulling full @types/node (which would widen the ambient surface for browser code).
// 若后续需完整 Node 类型，安装 @types/node 并移除此声明。
declare const process: undefined | { env?: { [key: string]: string | undefined } };
